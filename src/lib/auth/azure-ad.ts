...
export function validateAzureADConfig() {
  ...
  if (config.clientId && config.clientId.length < 32) {
    throw new Error('AZURE_AD_CLIENT_ID appears to be invalid (too short)');
  }
  
  if (config.clientSecret && config.clientSecret.length < 32) {
    throw new Error('AZURE_AD_CLIENT_SECRET appears to be invalid (too short)');
  }
  
  if (config.tenantId && config.tenantId.length < 32) {
    throw new Error('AZURE_AD_TENANT_ID appears to be invalid (too short)');
  }
